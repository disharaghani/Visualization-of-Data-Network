...
var cfnestedarr= new Array()
const tableBody=document.getElementById('flow-table')
const tableBody2=document.getElementById('flow-next-table')
...
var alg = 0; //0-ford fulkerson
document.getElementById("algorithm").onchange = function() { // sets algorithm type.
    if (document.getElementById("algorithm").value === "fordfulkerson")
        alg = 0;
    else alg = 1;
    mygraph.clearAlg();
}
...
function Queue() {
    ...
    this.enqueue = function(item) {
        this.arr.unshift(item); //unshift add elements at the start of array
    };
...
function ResidualEdge(u, v, c) {
    ...
    this.u = u; // from Vertex id --> source
    this.v = v; // to Vertex id --> Destination
    this.c = c; // capacity
...
/* Graph data structure and functions */
function graph() {
    this.vertices = [];
    this.edges = [];
...
function addData(arr){
    ...
    // cell2.textContent=document.getElementById("aug").value;
...
function addFlow(cf_para){
    console.log("add flow called!")
    console.log(cf_para)
    var table2 = tableBody2;
    var row = document.createElement('tr');
    var cell = document.createElement('td');
    cell.textContent = cf_para;
    row.appendChild(cell);
...
function play() {
    ...
    console.log(this.flag);
     if(this.flag ==  0){
         alert("Please choose all the nodes");
         return 0;
     }
    ...
    if (alg === 0) { // Edmonds-Karp algorithm
       ...
       /* if(mygraph.vertices.length  ==  false){
            alert("Please choose all the nodes");
            playing = false;
            return;
        }*/
